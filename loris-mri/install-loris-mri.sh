...
mysql ${MYSQL_DATABASE} -h${MYSQL_HOST} --user=${MYSQL_USER} --password="${MYSQL_PASSWORD}" -A -e "UPDATE Config SET Value='${DATA_DIR}/' WHERE ConfigID=(SELECT ID FROM ConfigSettings WHERE Name='dataDirBasepath')"
mysql ${MYSQL_DATABASE} -h${MYSQL_HOST} --user=${MYSQL_USER} --password="${MYSQL_PASSWORD}" -A -e "UPDATE Config SET Value='${DATA_DIR}/' WHERE ConfigID=(SELECT ID FROM ConfigSettings WHERE Name='imagePath')"
...
mysql ${MYSQL_DATABASE} -h${MYSQL_HOST} --user=${MYSQL_USER} --password="${MYSQL_PASSWORD}" -A -e "UPDATE Config SET Value='${DATA_DIR}/tarchive/' WHERE ConfigID=(SELECT ID FROM ConfigSettings WHERE Name='tarchiveLibraryDir')"
